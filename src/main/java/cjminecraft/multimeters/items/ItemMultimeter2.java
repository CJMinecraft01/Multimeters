...
public class ItemMultimeter2 extends Item {

	/**
	 * Initialize the item
	 */
	public ItemMultimeter2() {
	...
	public static class MultimeterOverlay extends GuiOverlay {
		...
		public MultimeterOverlay() {
			...
			OverlayBase itemOverlay = new OverlayInventory(this, MultimetersConfig.GUI.OFFSET_X,
					this.height - MultimetersConfig.GUI.OFFSET_Y).showOverlayText(false);
			itemOverlay.addElement(new ElementItemSlot(this, 0, 0));
			addOverlay(itemOverlay);

			OverlayBase fluidOverlay = new OverlayBase(this, MultimetersConfig.GUI.OFFSET_X,
					this.height - MultimetersConfig.GUI.OFFSET_Y - MultimetersConfig.GUI.FLUID_BAR_HEIGHT - 20)
							.showOverlayText(false);
			fluidOverlay.addElement(new ElementItemSlot(this, 0, MultimetersConfig.GUI.FLUID_BAR_HEIGHT + 1));
			fluidOverlay.setVisible(false);
			addOverlay(fluidOverlay);
			OverlayBase energyOverlay = new OverlayBase(this, MultimetersConfig.GUI.OFFSET_X,
					this.height - MultimetersConfig.GUI.OFFSET_Y - MultimetersConfig.GUI.ENERGY_BAR_HEIGHT - 20)
							.showOverlayText(false);
			energyOverlay.addElement(new ElementEnergyBar(this, 0, 0, MultimetersConfig.GUI.ENERGY_BAR_WIDTH,
					MultimetersConfig.GUI.ENERGY_BAR_HEIGHT));
			energyOverlay.addElement(new ElementItemSlot(this, 0, MultimetersConfig.GUI.ENERGY_BAR_HEIGHT + 1));
		...
		protected void updateElementInformation() {
			this.overlays.get(0).setPosition(MultimetersConfig.GUI.OFFSET_X,
					this.height - MultimetersConfig.GUI.OFFSET_Y - this.overlays.get(0).getHeight());
			if (this.overlays.get(0).isVisible()) {
				this.overlays.get(2).setPosition(this.overlays.get(0).getPosX() + this.overlays.get(0).getWidth() + 6,
						this.height - MultimetersConfig.GUI.OFFSET_Y - MultimetersConfig.GUI.ENERGY_BAR_HEIGHT - 27);
				if (this.overlays.get(2).isVisible())
					this.overlays.get(1).setPosition(
							...
							this.height - MultimetersConfig.GUI.OFFSET_Y - MultimetersConfig.GUI.FLUID_BAR_HEIGHT - 27);
				else
					this.overlays.get(1).setPosition(
							this.overlays.get(0).getPosX() + this.overlays.get(0).getWidth() + 6,
							this.height - MultimetersConfig.GUI.OFFSET_Y - MultimetersConfig.GUI.FLUID_BAR_HEIGHT - 27);
			} else {
				this.overlays.get(2).setPosition(MultimetersConfig.GUI.OFFSET_X,
						this.height - MultimetersConfig.GUI.OFFSET_Y - MultimetersConfig.GUI.ENERGY_BAR_HEIGHT - 27);
				if (this.overlays.get(2).isVisible())
					this.overlays.get(1).setPosition(
							this.overlays.get(2).getPosX() + this.overlays.get(2).getWidth() + 6,
					...
					this.overlays.get(1).setPosition(MultimetersConfig.GUI.OFFSET_X,
							this.height - MultimetersConfig.GUI.OFFSET_Y - MultimetersConfig.GUI.FLUID_BAR_HEIGHT - 27);
			}
			addInventoryOverlay();
			addEnergyOverlay();
		...
		private void addEnergyOverlay() {
			ItemStack energyMultimeter = InventoryUtils.findInHotbar(new ItemStack(MultimetersItems.multimeter, 1, 0),
			...
			energyBar.setSize(MultimetersConfig.GUI.ENERGY_BAR_WIDTH, MultimetersConfig.GUI.ENERGY_BAR_HEIGHT);
			itemSlot.setPosition(itemSlot.getPosX(), MultimetersConfig.GUI.ENERGY_BAR_HEIGHT + 1);
		...
		private void addInventoryOverlay() {
			ItemStack itemMultimeter = InventoryUtils.findInHotbar(new ItemStack(MultimetersItems.multimeter, 1, 1), player,
					true, false);
		...
		private void addFluidOverlay() {
			ItemStack fluidMultimeter = InventoryUtils.findInHotbar(new ItemStack(MultimetersItems.multimeter, 1, 2), player,
					true, false);
			...
			if(this.overlays.get(1).getElements().size() > 1)
				...
				itemSlot.setPosition(itemSlot.getPosX(), MultimetersConfig.GUI.FLUID_BAR_HEIGHT + 2);
			...
			if (fluidMultimeter.hasTagCompound() && fluidMultimeter.getTagCompound().hasKey("BlockPos")) {
				...
				if (this.overlays.get(1).getElements().size() - 1 != FluidUtils
					...
					for (int i = 0; i < FluidUtils.getNumberOfTanks(this.mc.world.getTileEntity(pos), side); i++)
						this.overlays.get(1)
								.addElement(new ElementFluidBar(this, i * MultimetersConfig.GUI.FLUID_BAR_WIDTH, 0, i)
										.shouldSync(pos, side).setSize(MultimetersConfig.GUI.ENERGY_BAR_WIDTH,
												MultimetersConfig.GUI.ENERGY_BAR_HEIGHT));
				} else {
					for (int i = 0; i < FluidUtils.getNumberOfTanks(this.mc.world.getTileEntity(pos), side); i++)
						((ElementFluidBar) this.overlays.get(1).getElements().get(i + 1)).shouldSync(pos, side)
								.setSize(MultimetersConfig.GUI.ENERGY_BAR_WIDTH, MultimetersConfig.GUI.ENERGY_BAR_HEIGHT);
				}
				ItemStack block = getStackFromBlock(pos, side);
				if (!InventoryUtils.isStackEqual(block, itemSlot.getStack(), true, false))
					itemSlot.setStack(block);
							...
							this.overlays.get(1)
									.addElement(new ElementFluidBar(this, i * MultimetersConfig.GUI.FLUID_BAR_WIDTH, 0, i)
											.shouldSync(result.getBlockPos(), result.sideHit).setSize(
													MultimetersConfig.GUI.FLUID_BAR_WIDTH,
													MultimetersConfig.GUI.FLUID_BAR_HEIGHT));
					} else {
						for (int i = 0; i < FluidUtils.getNumberOfTanks(
								this.mc.world.getTileEntity(result.getBlockPos()), result.sideHit); i++)
							((ElementFluidBar) this.overlays.get(1).getElements().get(i + 1))
									.shouldSync(result.getBlockPos(), result.sideHit)
									.setSize(MultimetersConfig.GUI.FLUID_BAR_WIDTH,
											MultimetersConfig.GUI.FLUID_BAR_HEIGHT);
					}
					ItemStack block = getStackFromBlock(result.getBlockPos(), result.sideHit);
					if (!InventoryUtils.isStackEqual(block, itemSlot.getStack(), true, false))
					...
					for (int i = 0; i < FluidUtils.getNumberOfTanks(this.player.getHeldItemMainhand(), null); i++)
						((ElementFluidBar) this.overlays.get(1).getElements().get(i + 1))
								.setFluidTankInfo(new FluidTankInfo(
										FluidUtils.getFluidStack(this.player.getHeldItemMainhand(), null, i),
										FluidUtils.getCapacity(this.player.getHeldItemMainhand(), null, i)))
								.setSize(MultimetersConfig.GUI.FLUID_BAR_WIDTH, MultimetersConfig.GUI.FLUID_BAR_HEIGHT);
				}
				itemSlot.setStack(this.player.getHeldItemMainhand());
		 ...
		 * @param side
		 *            The targetSide of the block (for use with
